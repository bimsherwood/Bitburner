...
export function StorageUpgrade(ns, options){
  ...
  async function getSize(){
    return await ns.corporation.getWarehouse(division, city).size;
  }
  
  async function getUpgradeCost(){
    return await ns.corporation.getUpgradeWarehouseCost(division, city);
  }
  
  async function upgrade(){
    var sizeBefore = await getSize();
    await ns.corporation.upgradeWarehouse(division, city);
    var sizeAfter = await getSize();
